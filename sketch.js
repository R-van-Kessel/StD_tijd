...
function generateQuestions() {
    ...
    for (let i = 0; i < 10; i++) {
        // Kies type vraag: 1 = minuten naar uren, 2 = kwartier naar uren, 
        // 3 = uren naar minuten, 4 = uren naar kwartier,
        // 5 = seconden naar minuten, 6 = minuten naar seconden
        const questionType = floor(random(1, 7));
        
        let text, answer;
        
        if (questionType === 1) {
            // Minuten naar uren - start met uren in kwartieren
            let quarters = floor(random(1, 40)); // 1-40 kwartier
            let minutes = quarters * 15; // Altijd deelbaar door 15
            let hours = quarters * 0.25; // Precies in kwartieren
            
            text = `${minutes} minuten\n= ? uur`;
            answer = `${hours}`.replace('.', ',');
            
        } else if (questionType === 2) {
            // Kwartier naar uren
            let quarters = floor(random(2, 40)); // 2-40 kwartier
            let hours = quarters * 0.25; // Precies deelbaar
            
            text = `${quarters} kwartier\n= ? uur`;
            answer = `${hours}`.replace('.', ',');
            
        } else if (questionType === 3) {
            // Uren naar minuten - start met kwartieren
            let quarters = floor(random(1, 32)); // 1-32 kwartier
            let hours = quarters * 0.25;
            let minutes = quarters * 15; // Altijd geheel
            
            text = `${hours}`.replace('.', ',') + ` uur\n= ? minuten`;
            answer = `${minutes}`;
            ...
            // Seconden naar minuten - start met minuten in 0.25 stappen
            let quarterMinutes = floor(random(1, 40)); // 1-40 keer 0.25 minuut
            let seconds = quarterMinutes * 15; // Altijd deelbaar door 15
            let minutes = quarterMinutes * 0.25; // Precies in kwartminuten
            
            text = `${seconds} seconden\n= ? minuten`;
            answer = `${minutes}`.replace('.', ',');
            
        } else {
            // Minuten naar seconden - start met minuten in 0.25 stappen
            let quarterMinutes = floor(random(1, 40)); // 1-40 keer 0.25 minuut
            let minutes = quarterMinutes * 0.25;
            let seconds = quarterMinutes * 15; // Altijd geheel
            
            text = `${minutes}`.replace('.', ',') + ` minuten\n= ? seconden`;
            answer = `${seconds}`;
        }
    ...
    // Maak vraag-blokken (rijen 0 en 1)
    let questionIndex = 0;
    ...
    // Maak antwoord-blokken (rijen 2 en 3)
    let answerIndex = 0;
...
function drawBlock(block) {
    ...
    // Tekstgrootte
    if (block.isQuestion) {
        textSize(14);  // Vaste grootte voor vragen
    } else {
        // Antwoordblokjes: pas aan op basis van lengte
    ...
    // Als de tekst \n bevat, teken dan meerdere regels
    if (block.text.includes('\n')) {
        let lines = block.text.split('\n');
        let lineHeight = 18;  // Ruimte tussen regels
        let startY = block.y + CELL_SIZE / 2 - (lines.length - 1) * lineHeight / 2;
        
        for (let i = 0; i < lines.length; i++) {
            text(lines[i], block.x + CELL_SIZE / 2, startY + i * lineHeight);
        }
    } else {
        // Enkele regel
